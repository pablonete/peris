...
- **Use `model/<model>/` for model-specific multi-file logic**: When a set of business-logic files belongs to one domain model, group them under a dedicated root folder such as `model/cashflow/` instead of spreading them across `lib/`.
